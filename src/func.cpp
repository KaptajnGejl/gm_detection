...
corner find_square(vector<corner> corners, Mat& img_cor, Mat& img, float limit, float center_is_black_limit) {
				...
				for (unsigned int k = j+1; k < corners.size(); k++)
				{
					...
					for (unsigned int l = k+1; l < corners.size(); l++)
					{
						...
						if ((match < best_match) && (match < limit) && (match != 0))  {
							...
							if (center_check(temp_center,hist(img,false),img,center_is_black_limit)) { //&& similar_point_check(img_thr,temp)) {
								best_match = match;
								result = temp;
							}
						}
...
corner find_triangle(vector<corner> corners, Mat& img_cor, Mat& img, float limit, float center_is_black_limit) {
			...
			for (unsigned int j = i+1; j < corners.size(); j++)
			{
				...
				for (unsigned int k = j+1; k < corners.size(); k++)
				{
					...
					if ((match < best_match) && (match < limit) && (match != 0)) {
						...
						if (center_check(temp_center,hist(img,false),img,center_is_black_limit)) { 
							best_match = match;
							result = temp;
						}
					}
